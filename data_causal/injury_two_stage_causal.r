...
# Two-stage causal injury analysis.
# Stage 1 (MAP): aging curve fit on healthy observations only; survival censored
#   at injury onset → counterfactual plug-ins (X, beta, AR, survival params).
# Stage 2 (MCMC): injury parameters sampled against the fixed Stage 1 baseline.
# posterior_counterfactual_ar in the Stage 2 output IS the Stage 1 baseline
# (Stage 1 fixed curve + AR, injury effect = 0).
args      <- commandArgs(trailingOnly = TRUE)
model_dir <- if (length(args) >= 1) args[1] else stop("Usage: Rscript injury_two_stage_causal.r <model_dir>")
...
plot_counterfactual <- function(grouped_data_set) {
  ...
  plt <- raw_plt |>
    ...
    labs(x = "Age", y = "Metric Value") + ggtitle(paste("Two-Stage Counterfactual Trajectory (Stage 1 baseline): ", group_name)) + theme_bw()
  return(plt)
}
...
filter(injury_period == "post-injury" & year <= 2023 & metric %in% c("MPG", "FTA", "OBPM", "USG")) %>%
  group_by(player) %>%
  group_split() %>%               # splits into a list of grouped tibbles
  map(~ {
