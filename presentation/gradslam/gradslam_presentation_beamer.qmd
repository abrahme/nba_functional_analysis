...
# Missing Data Challenges {.smaller}
- Missing data due to players not playing every season, and some metrics only observed when players are active.
- Right censored data for players who are still active or whose careers end after 38.
- Left censored data for players who start after age 18 or whose careers start before 1997.
- Mix of informative and uninformative missingness patterns across metrics and seasons.
...
{\color{orange}{\eta^k_{pt}}} &= \mu^k_{pt}
 + \mathbb{1}_{\{k\in\mathcal{K}_{\text{rate}}\}}\log e^k_{pt} \\
{\color{orange}{Y^k_{pt}}} &\sim \operatorname{EF}\!\left({\eta^k_{pt}},\, {\color{orange}{\phi^k_{pt}}}\right)
\end{aligned}
$$
}
...
- Bayesian functional model for joint skill + athleticism trajectories.
- Handles heteroscedastic uncertainty, concave aging structure, and latent player embeddings.
- Explicitly estimates injury impact and counterfactual recovery paths.
- Metric correlation is not explicitly modeled, but shared latent structure induce correlation in practice.
...
- A rotation matrix $R$ can be applied to the latent space such that $\alpha_l(X) = w_l^T R R^{-1} X$, which preserves the likelihood but creates non-identifiability between $w_l$ and $X$.
...
![Derrick Rose injury treatment effect by metric.](../../model_output/model_plots/causal/Derrick%20Rose_metrics_itt.png){width="82%"}



---

# Injury Impact Estimation: Population Posterior
