...
ls_deriv = results_tvrflvm_svi["lengthscale_deriv__loc"]
alpha_time = results_tvrflvm_svi["alpha__loc"]
shifted_x_time = hsgp_params["shifted_x_time"]
spd = jnp.sqrt(diag_spectral_density(1, alpha_time, ls_deriv, L_time, M_time))
weights = results_tvrflvm_svi["beta__loc"]
weights = weights * spd * .0001
W = results_tvrflvm_svi["W__loc"] 
wTx = jnp.einsum("nr,mr -> nm", X, W)
psi_x = jnp.hstack([jnp.cos(wTx), jnp.sin(wTx)]) * (1/ jnp.sqrt(50))
slope = make_psi_gamma(psi_x, results_tvrflvm_svi["slope__loc"])
intercept = make_psi_gamma(psi_x, results_tvrflvm_svi["intercept__loc"])
...
core, factors = tucker(mu, rank= [16, X.shape[0], 3])
print(core.shape)
for factor in factors:
    print(factor.shape)
full_core = np.einsum("nkl,lj -> nkj", np.einsum("nc,kcl -> nkl", factors[0], core), factors[1])
...
for pos in positions:
    core, factors = tucker(mu[:,pos_indices[pos_indices["position_group"] == "G"].index.values,:],
                        rank = [16, X.shape[0], 3])
    full_pos_core = np.einsum("nkl,lj -> nkj", np.einsum("nc,kcl -> nkl", factors[0], core), factors[1])
